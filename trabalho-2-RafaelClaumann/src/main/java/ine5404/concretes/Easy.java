 ...
 * @author rafael
 */
public final class Easy implements IBehaviour {
    ...
    public Easy(int dimensions, int wordsQuantity) throws IOException, FileNotFoundException, ClassNotFoundException, ESizeLimiter {
        startGame(dimensions, wordsQuantity);
    }

    @Override
    public void startGame(int dimensions, int wordsQuantity) throws IOException, FileNotFoundException, ClassNotFoundException, ESizeLimiter {
        board = new char[dimensions][dimensions];
        fillRandomBoard();
        readWordsFromFile();

        ArrayList<String> words1 = sortWords(wordsQuantity / 2);
        insertVerticalNormal(words1);

        ArrayList<String> words2 = sortWords(wordsQuantity / 2);
        insertWords(words2);
    }
    ...
    public ArrayList<String> sortWords(int quantity) throws ESizeLimiter {
        ...
        if (board.length <= 16 && (2 * quantity) > 6) {
            throw new ESizeLimiter();
        } else if (board.length == 20 && (2 * quantity) > 8) {
            throw new ESizeLimiter();
        }
